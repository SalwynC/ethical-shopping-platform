import { Body, Controller, Get, Header, Post, Query, Logger, Inject, Req } from '@nestjs/common';
...
type PriceTrend = {
  ...
  competitorPrices?: Array<{platform: string; price: number; confidence: number}>;
};
...
type AnalysisResult = {
  ...
  priceAnalysis: {
    ...
    priceRank: 'lowest' | 'below_average' | 'average' | 'above_average' | 'highest';
...
export class AppController {
  ...
  async analyze(body: AnalyzeDto): Promise<AnalysisResult> {
    ...
    try {
      ...
      // Step 3: AI Analysis
      const aiAnalysis: AIAnalysis = await this.aiService.analyzeProduct(productData, body.url);
  ...
  private calculatePriceRank(currentPrice: number, originalPrice?: number): 'lowest' | 'below_average' | 'average' | 'above_average' | 'highest' {
  ...
  private calculateTrustScore(productData: ProductData, aiAnalysis: AIAnalysis): {
    dataReliability: number;
    sourceConfidence: number;
    overallTrust: 'high' | 'medium' | 'low';
    explanation: string;
    ...
    if (productData.originalPrice && productData.originalPrice > productData.price) reliability += 15;
    if (productData.rating && productData.rating > 0) reliability += 10;
    if (productData.reviewCount && productData.reviewCount > 10) reliability += 10;
    
    // Source confidence based on platform
    const sourceConfidence = productData.title?.includes('Apple') ? 85 : 
                           productData.title?.includes('Samsung') ? 80 : 70;
  ...
  private getTrustExplanation(trustLevel: string, hasReliableData: boolean): string {
    if (trustLevel === 'high') {
  ...
  private determineAvailability(productData: ProductData): 'in_stock' | 'out_of_stock' | 'limited' | 'unknown' {
    if (productData.price > 0 && productData.title) return 'in_stock';
    if (productData.price === 0) return 'out_of_stock';
    return 'unknown';
  }

  private getEnhancedDecision(dealScore: number, ethicalScore: number, reliability: number): 'buy_now' | 'wait' | 'avoid' | 'insufficient_data' {
  ...
  private getRecommendationAction(dealScore: number, priceRank: string): string {
  ...
  private getRecommendationReasoning(dealScore: number, priceRank: string, isGoodDeal: boolean): string[] {
    const reasoning = [];
    
    if (isGoodDeal) {
      reasoning.push('Deal score indicates excellent value');
      if (priceRank === 'lowest') reasoning.push('Currently at lowest observed price point');
    } else {
      reasoning.push('Deal score suggests waiting for better offers');
    }
    
    if (priceRank === 'highest') reasoning.push('Price is above market average');
    if (priceRank === 'below_average') reasoning.push('Price is competitive with market');
  ...
  private generateCompetitorPrices(currentPrice: number, platform: string): Array<{platform: string; price: number; confidence: number}> {
  ...
  private generateHonestAssessment(aiAnalysis: AIAnalysis, trustScore: any): string {
  ...
  private generateAlternatives(productData: ProductData, aiAnalysis: AIAnalysis): AlternativeProduct[] {
  ...
  private generateEthicalRules(aiAnalysis: AIAnalysis): EthicalRuleExplanation[] {
    return [
      {
  ...
  private async logAnalysis(url: string, platform: string, dealScore: number, ethicalScore: number): Promise<void> {
    try {
      // Update analytics
  ...
  private getDecision(dealScore: number, ethicalScore: number): 'buy_now' | 'wait' | 'insufficient_data' {
    const avgScore = (dealScore + ethicalScore) / 2;
  ...
  private convertToEthicalRules(aiAnalysis: AIAnalysis): EthicalRuleExplanation[] {
