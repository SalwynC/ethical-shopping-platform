...
export class AIService {
  ...
  async analyzeProduct(productData: ProductData, url: string): Promise<AIAnalysis> {
  ...
  private async performAIAnalysis(productData: ProductData, url: string): Promise<AIAnalysis> {
  ...
  private async analyzeWithOpenAI(productData: ProductData, url: string): Promise<AIAnalysis> {
...
Respond with ONLY valid JSON, no additional text.`;
  ...
  private async analyzeWithGemini(productData: ProductData, url: string): Promise<AIAnalysis> {
  ...
  private validateAndEnhanceAIResponse(aiResponse: any, productData: ProductData): AIAnalysis {
    ...
    return {
      ...
      priceAnalysis: {
        ...
        recommendation: aiResponse.priceAnalysis?.recommendation || 'Monitor price trends before purchasing',
        confidence: Math.max(0, Math.min(100, aiResponse.priceAnalysis?.confidence || 70)),
      },
      insights: {
        keyStrengths: aiResponse.insights?.keyStrengths || ['Product available for analysis'],
        ...
        recommendation: aiResponse.insights?.recommendation || 'Consider your specific needs and budget',
      },
      sustainability: {
        score: Math.max(0, Math.min(100, aiResponse.sustainability?.score || 50)),
        factors: aiResponse.sustainability?.factors || ['General sustainability considerations'],
        impact: aiResponse.sustainability?.impact || 'Environmental impact assessment in progress',
      },
  ...
  private getFallbackAnalysis(productData: ProductData): AIAnalysis {
    ...
    // Data quality bonus
    if (productData.description && productData.description.length > 100) ethicalScore += 5;
  ...
  private inferMarketPosition(productData: ProductData): 'excellent' | 'good' | 'average' | 'poor' {
  ...
  private generateFallbackAlternatives(productData: ProductData): Array<{title: string; price: number; score: number; reason: string}> {
  ...
  private validateMarketPosition(position: any): 'excellent' | 'good' | 'average' | 'poor' {
    const validPositions = ['excellent', 'good', 'average', 'poor'];
    return validPositions.includes(position) ? position : 'average';
  }

  private validatePriceHistory(history: any): 'rising' | 'stable' | 'declining' {
  ...
  private generatePriceRecommendation(productData: ProductData, dealScore: number): string {
    if (dealScore >= 85) {
  ...
  private generateOverallRecommendation(dealScore: number, ethicalScore: number): string {
