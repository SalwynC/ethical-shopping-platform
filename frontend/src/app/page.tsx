  ...
  useEffect(() => {
    ...
    const validateUrl = () => {
      ...
      try {
        ...
        const isSupported = supportedSites.some(
          (site) =>
            hostname === site ||
            hostname === `www.${site}` ||
        ...
        const hasProductPath =
          /\/(product|item|p|dp|gp)\/|\/(.*\/)?(\w+-)*(\d+)/i.test(
            url.pathname,
          );
        ...
        if (isSupported || hasProductPath || hasProductParams) {
          ...
          setUrlError(
            `This doesn't look like a product page. Try URLs from Amazon, eBay, Nike, etc.`,
          );
          setIsValidUrl(false);
        }
      } catch (error) {
        ...
        if (urlString.includes(' ') || !urlString.includes('.')) {
          setUrlError(
            'Please enter a complete URL (e.g., https://amazon.com/product)',
          );
        } else {
          setUrlError(
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `HTTP ${response.status}: Product analysis failed`,
        );
      }

      const analysisData = await response.json();
      console.log(
        `✅ [${requestId}] Analysis completed in ${processingTime}ms`,
      );
      ...
      const mockResults = {
        ...
        productName:
          analysisData.productInfo?.title ||
          `Product from ${new URL(productUrl.trim()).hostname}`,
        overallScore: Math.round(
          (analysisData.ethicalScore + analysisData.dealScore) / 2,
        ),
        ethicalScore: analysisData.ethicalScore || 75,
        priceScore: analysisData.dealScore || 70,
        sustainabilityScore: Math.round(
          analysisData.ethicalScore * 0.8 + analysisData.dealScore * 0.2,
        ),
        reviewTrust: analysisData.productInfo?.rating
          ? Math.round(analysisData.productInfo.rating * 20)
          : 80,
        processingTime,
        requestId,
        timestamp: new Date().toISOString(),
        ...
        futurePredict: {
          ...
          bestTimeToByy:
            analysisData.priceTrend?.marketTrend === 'declining'
              ? 'Soon - Price likely to drop further'
              : 'Now - Good deal',
          priceVolatility: analysisData.priceTrend?.confidence
            ? 'Low volatility'
            : 'High volatility',
        },
      };
          ...
          <div className="max-w-6xl mx-auto px-6">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                >
                  <SavedToday
                    amountUsd={isClientMounted ? liveStats.saved : 750}
                  />
                  <div className="text-gray-700 dark:text-gray-200 text-sm font-medium">
