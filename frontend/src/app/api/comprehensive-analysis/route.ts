...
const comprehensiveAnalytics: AdvancedAnalytics[] = [];
const timeBasedStatistics: { [period: string]: TimeBasedStats } = {};
    ...
    if (action === 'comprehensive_analysis') {
      ...
      try {
        ...
        if (backendResponse.ok) {
          ...
          console.log(
            ...
            realProductData.productInfo?.title || realProductData.title,
          );
        } else {
          const errorText = await backendResponse.text();
          console.warn(
            `⚠️ Backend scraping failed (${backendResponse.status}):`,
        ...
        console.log(
          `- Running in: ${typeof window === 'undefined' ? 'server' : 'client'} context`,
        );
      ...
      const advancedAnalysis: AdvancedAnalytics = {
        productData: {
          ...
          brand:
            realProductData?.productInfo?.brand ||
            extractBrandFromUrl(productData.url),
          category:
            realProductData?.productInfo?.category ||
            determineCategory(productData.url, productData.title),
          price:
            realProductData?.priceTrend?.current ||
            generateRealisticPrice(productData.category),
          currency: realProductData?.priceTrend?.currency || 'INR',
          marketplace: new URL(productData.url).hostname,
          availability:
            realProductData?.productInfo?.availability || 'in_stock',
        },
        ...
        marketResearch: {
          competitorAnalysis: {
            ...
            pricePosition:
              Math.random() > 0.5 ? 'below_market' : 'above_market',
        ...
        ethicalAssessment: {
          overallScore:
            realProductData?.ethicalScore ||
            analysisResults?.ethicalScore ||
            Math.floor(Math.random() * 100) + 1,
          laborPractices: {
            score:
              realProductData?.sustainability?.brandRating ||
              Math.floor(Math.random() * 100) + 1,
            certifications: generateCertifications(),
            workingConditions: ['excellent', 'good', 'fair', 'poor'][
          ...
          environmentalImpact: {
            score:
              realProductData?.sustainability?.ethicalSourcing ||
              Math.floor(Math.random() * 100) + 1,
            carbonFootprint: Math.random() * 100,
            sustainabilityRating:
              realProductData?.sustainability?.environmentalImpact ||
        ...
        timeBasedAnalytics: {
          ...
          nextUpdateRecommended: new Date(
      ...
      console.log(
        `🚀 COMPREHENSIVE ANALYSIS COMPLETED: ${advancedAnalysis.productData.title}`,
      );
      console.log(
        `📊 Market Research: ${advancedAnalysis.marketResearch.competitorAnalysis.similarProducts} competitors found`,
      );
      console.log(
        `🌱 Ethical Score: ${advancedAnalysis.ethicalAssessment.overallScore}/100`,
      );
      console.log(
        `⚠️ Risk Level: ${advancedAnalysis.riskAssessment.overallRisk}`,
      );
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (analysisId) {
      ...
      const analysis = comprehensiveAnalytics.find(
        (a) => a.timeBasedAnalytics.analysisTimestamp === analysisId,
      );
...
function determineCategory(url: string, title: string): string {
  ...
  if (lowerTitle.includes('shirt') || lowerTitle.includes('clothing'))
    return 'clothing';
  if (lowerTitle.includes('shoe') || lowerTitle.includes('sneaker'))
    return 'footwear';
  if (lowerTitle.includes('phone') || lowerTitle.includes('iphone'))
    return 'electronics';
  if (lowerTitle.includes('coffee') || lowerTitle.includes('food'))
    return 'food';
  return 'general';
}
...
function generatePriceHistory(): Array<{
  date: string;
  price: number;
  source: string;
}> {
  ...
  return [
    {
      ...
      reasonForRecommendation:
        'Better labor practices and environmental impact',
    },
    {
      ...
      reasonForRecommendation:
        'Fair trade certified with transparent supply chain',
    },
  ];
}

function updateTimeBasedStats(analysis: AdvancedAnalytics) {
  const periods: Array<'24h' | '7d' | '30d' | '90d' | '6m' | '1y'> = [
    '24h',
    '7d',
    '30d',
    '90d',
  ...
  periods.forEach((period) => {
    ...
    let categoryEntry = stats.metrics.topCategories.find(
    ...
    categoryEntry.averageScore =
      (categoryEntry.averageScore + analysis.ethicalAssessment.overallScore) /
      2;
  });
}
