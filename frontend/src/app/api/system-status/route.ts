...
class RealAnalyticsTracker {
  ...
  private activeBrowserSessions = new Map<string, { lastSeen: number; userAgent: string; }>();
...
async function checkRealServiceHealth(serviceName: string, url?: string): Promise<{
  status: 'online' | 'offline';
  responseTime: number;
  lastPing: string;
}> {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [frontendCheck, backendCheck, analyticsCheck, dbCheck] = await Promise.allSettled([
      checkRealServiceHealth('Frontend Server'), // Always online if this API responds
      checkRealServiceHealth('Backend API', 'http://localhost:4000/api/health'),
      checkRealServiceHealth('Analytics Service', 'http://localhost:8001/health'),
    ...
    const services = {
      ...
      'Backend API': {
        status: (backendCheck.status === 'fulfilled' ? backendCheck.value.status : 'offline') as 'online' | 'offline',
        responseTime: backendCheck.status === 'fulfilled' ? backendCheck.value.responseTime : 0,
        lastPing: backendCheck.status === 'fulfilled' ? backendCheck.value.lastPing : new Date().toISOString()
      },
      'Analytics Service': {
        status: (analyticsCheck.status === 'fulfilled' ? analyticsCheck.value.status : 'offline') as 'online' | 'offline',
        responseTime: analyticsCheck.status === 'fulfilled' ? analyticsCheck.value.responseTime : 0,
        lastPing: analyticsCheck.status === 'fulfilled' ? analyticsCheck.value.lastPing : new Date().toISOString()
      },
      'Database': {
        status: (dbCheck.status === 'fulfilled' ? dbCheck.value.status : 'offline') as 'online' | 'offline',
        responseTime: dbCheck.status === 'fulfilled' ? dbCheck.value.responseTime : 0,
        lastPing: dbCheck.status === 'fulfilled' ? dbCheck.value.lastPing : new Date().toISOString()
      }
    };
    
    // Calculate real system health
    const onlineServices = Object.values(services).filter(s => s.status === 'online').length;
    const systemHealth = Math.round((onlineServices / Object.keys(services).length) * 100);
