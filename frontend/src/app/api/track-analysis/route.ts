...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      totalAnalyses: totalAnalysesCount,
      analysesToday: analysisLogs.filter(
        (analysis) =>
          new Date(analysis.timestamp).toDateString() ===
          new Date().toDateString(),
      ).length,
      averageProcessingTime:
        analysisLogs.length > 0
          ? Math.round(
              analysisLogs.reduce((sum, a) => sum + a.processingTime, 0) /
                analysisLogs.length,
            )
          : 0,
      categoriesAnalyzed: Array.from(
        new Set(analysisLogs.map((a) => a.category)),
      ).length,
      averageEthicalScore:
        analysisLogs.filter((a) => a.ethicalScore !== undefined).length > 0
          ? Math.round(
              analysisLogs
